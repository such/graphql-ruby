  ...
  module Relay
    ...
    module ConnectionInstrumentation
      ...
      def self.instrument(type, field)
        should_instrument = if field.connection?
          if (field_cls = field.metadata[:type_class])
            # Don't apply the wrapper to class-based field with a the connection extension
            field_cls.extensions.empty?
          else
            true
          end
        else
